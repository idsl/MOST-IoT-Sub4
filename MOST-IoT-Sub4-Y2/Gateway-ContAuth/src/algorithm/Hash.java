...
public class Hash {
    ...
    private static final String SHA_ALGORITHM=SHA1_ALGORITHM; //The used algorithm will be sha1
    ...
    public static byte[] getXorWithSHA(byte[] sha, int x)
    {
        ...
        byte[] bytes = ByteBuffer.allocate(4).putInt(x).array(); //transder x into four bytes
        byte[] result=Arrays.copyOfRange(sha,0,20); //The input array is long 20, get the last four
        for(int i=0;i<4;i++)
        {
            result[i+SHA_SIZE-4] =(byte)(sha[i+SHA_SIZE-4]^bytes[i]);
            //16 17 18 19 do XOR with x in byte form
        }
        return result; //return 20 byte, last 4 bytes have been changed
    }

    public static int getSubSHA(byte[] sha)
    {
        //get the last 4 bits, add and transfer to int!
        //Example: SHA_SIZE 20, sha[16]+sha[17]+sha[18]+sha[19] = int result!
